...
async function print_mitre() {
    ...
    iframe.onerror = function() {
        console.error('Failed to load MITRE iframe');
        Swal.fire({
            icon: 'error',
            title: 'Loading Error',
            text: 'Failed to load MITRE ATT&CK Navigator. Check console for details.',
        });
    };
    ...
    iframe.onload = function() {
        ...
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'CORS Error',
                text: 'Cannot access MITRE Navigator content. Try opening the page through a web server instead of file:// protocol.',
            });
            return;
        }

        // Auto-load the generated layer into Navigator (DOM fallback)
        (async () => {
            try {
                const layerData = JSON.parse(localStorage.getItem('layer'));
                if (!layerData) return;

                // 1) Click "Open Existing Layer"
                const clickOpen = () => {
                    const buttons = Array.from(iframeDoc.querySelectorAll('button'));    
                    const openBtn = buttons.find(b => /open\s+existing\s+layer/i.test(b.textContent || ''));
                ...
                const input = await new Promise(resolve => {
                    const tryFind = () => {
                        const el = iframeDoc.querySelector('input[type="file"]');
                ...
                // 3) Create a File from the JSON and dispatch change
                const json = JSON.stringify(layerData, null, 2);
                const file = new File([json], 'layer.json', { type: 'application/json' });
                const dt = new DataTransfer();
                dt.items.add(file);
                input.files = dt.files;
                input.dispatchEvent(new Event('change', { bubbles: true }));
            } catch (e) {
                console.error('ATT&CK auto-load failed:', e);
            }
        })();
